...
function canUserAccessLidarrAutomation(config, user) {
  ...
  const hasQuota = Number(quota.weeklyArtists || 0) !== 0 || Number(quota.weeklyAlbums || 0) !== 0;
  if (!hasQuota) return false;
  if (role !== 'power-user') return true;
  ...
  if (!started) {
    ...
    const _jobFunctions = {
      ...
      trackAnalysisPipeline: () => _routeCtx.trackEnrichmentService?.runAutomatedAnalysis({
        ...
        chunkSize: loadConfig().analysis?.analyzerChunkSize || 0,
        chunkDelayMs: loadConfig().analysis?.analyzerChunkDelayMs || 0,
        trackDelayMs: loadConfig().analysis?.analyzerTrackDelayMs || 0,
      }),
