...
export function createLidarrService(ctx) {
  ...
  function normalizeRole(role) {
    ...
    if (['admin', 'co-admin', 'power-user', 'user', 'guest', 'disabled'].includes(raw)) return raw;
    return 'user';
  }

  function hasRoleAutomationQuota(role, settings = getSettings()) {
    const roleKey = normalizeRole(role);
    if (roleKey === 'guest' || roleKey === 'disabled') return false;
    const quota = settings?.roleQuotas?.[roleKey] || { weeklyArtists: 0, weeklyAlbums: 0 };
    return Number(quota.weeklyArtists || 0) !== 0 || Number(quota.weeklyAlbums || 0) !== 0;
  }

  function resolveRelatedUserIdentifiers(userPlexId) {
    const value = String(userPlexId || '').trim();
    if (!value) return [];
    const config = loadConfig();
    const related = new Set([value]);
    const tokenEntries = Object.entries(
      config?.plex?.userServerTokens && typeof config.plex.userServerTokens === 'object'
        ? config.plex.userServerTokens
        : {},
    )
      .map(([id, token]) => [String(id || '').trim(), String(token || '').trim()])
      .filter(([id]) => Boolean(id));
    const matchingTokens = new Set(
      tokenEntries
        .filter(([id]) => id.toLowerCase() === value.toLowerCase())
        .map(([, token]) => token)
        .filter(Boolean),
    );
    if (!matchingTokens.size) return [...related];
    for (const [id, token] of tokenEntries) {
      if (token && matchingTokens.has(token)) related.add(id);
    }
    return [...related];
  }
  ...
  function resolveAutomationRoleForUserId(userPlexId) {
    ...
    if (typeof resolveRole === 'function') {
      for (const identity of identities) {
        const resolved = normalizeRole(resolveRole({
          username: identity,
          email: identity,
          title: identity,
        }));
  ...
  async function reviewArtistProgression(options = {}) {
    ...
    if (!hasRoleAutomationQuota(role, settings)) {
  ...
  async function reviewDueArtists(options = {}) {
    ...
    for (const item of due) {
      try {
        ...
        if (!hasRoleAutomationQuota(role, settings)) continue;
